...
onAddGcode : function(addGcodeCallback, gcodeParts, eagleWidget, helpDesc){ 
    ...
    addGcodeCallback(1500, myOwnGcode ); 
            ...
            while (!didLastPass) { //newZ > this.depthOfDimensions) {
                ...
                this.clipperDimension.forEach(function(pt) {
                    ...
                    if (pt.X == lastPt.X && pt.Y == lastPt.Y) {
                        ...
                        g += that.exportPCBDimensionHolder(pt, lastPt, newZ);
                    }
                    lastPt = pt;
                });
                // move to first point
        ...
        exportPCBDimensionHolder: function(point, lastpoint, z){
...
console.log('exportPCBDimensionHolder', this.PCBHolderLength, this.PCBHolderCount);
            ...
            for(var c=0;c < this.PCBHolderCount;c++){
                if(Math.abs(distance.segment.x) > this.PCBHolderLength)
                    x += distance.segment.x - this.PCBHolderLength;
                else
                    x += distance.segment.x;

                if(Math.abs(distance.segment.y) > this.PCBHolderLength)
                    y += distance.segment.y - this.PCBHolderLength;
                else
                ...
                // move to next plunge
                if(Math.abs(distance.segment.x) > this.PCBHolderLength)
                    (distance.segment.x < 0 ? x -= this.PCBHolderLength : x += this.PCBHolderLength);
                if(Math.abs(distance.segment.y) > this.PCBHolderLength)
                    (distance.segment.y < 0 ? y -= this.PCBHolderLength : y += this.PCBHolderLength);
